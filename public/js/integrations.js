...
(function initIntegrations() {
    ...
    async function checkGmailStatus() {
        ...
        try {
            const response = await fetch(`${API_BASE}/gmail/status`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } });
            if (!response.ok) { console.warn(`⚠️ Gmail durum kontrolü başarısız (${response.status}).`); return; }
            const data = await response.json();
            setGmailConnected(button, data.connected === true);
        } catch (error) { console.warn('⚠️ Gmail durum kontrolü yapılamadı:', error); }
    }

    async function connectGmail(button) {
        const token = getToken();
        if (!token) {
            if (window.Swal) await Swal.fire({ icon:'warning', title:'Giriş gerekli', text:'Gmail hesabını bağlamak için önce ChatChip hesabına giriş yap.' });
            else alert('Gmail hesabını bağlamak için önce giriş yap.');
            return;
        }
        if (button.dataset.connected === 'true') return;
        const originalText = button.textContent;
        button.disabled = true; button.textContent = 'Bağlanıyor...';
        try {
            const response = await fetch(`${API_BASE}/gmail/connect`, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization':`Bearer ${token}` } });
            let data = {}; try { data = await response.json(); } catch (_) {}
            if (!response.ok) throw new Error(data.error || `Gmail bağlantısı başlatılamadı (${response.status}).`);
            if (data.connected === true) { setGmailConnected(button, true); return; }
            if (!data.redirectUrl) throw new Error('Gmail yetkilendirme adresi alınamadı.');
            window.location.assign(data.redirectUrl);
        } catch (error) {
            console.error('❌ Gmail bağlantı hatası:', error);
            if (window.Swal) await Swal.fire({ icon:'error', title:'Gmail bağlanamadı', text:error.message || 'Bağlantı başlatılırken bir hata oluştu.' });
            else alert(error.message || 'Gmail bağlantısı başlatılırken bir hata oluştu.');
