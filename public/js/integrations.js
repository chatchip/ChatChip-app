...
(function initIntegrations() {
    ...
    function openAiEmployee(){const token=getToken();if(!token){if(window.Swal)Swal.fire({icon:'warning',title:'Giriş gerekli',text:'AI çalışanını kullanmak için önce ChatChip hesabına giriş yap.'});else alert('AI çalışanını kullanmak için önce giriş yap.');return;}if(aiEmployee?.id!=null){window.location.assign(`/ai-employee-manage.html?id=${encodeURIComponent(aiEmployee.id)}`);return;}window.location.assign('/ai-employee.html');}
